...
[source,js]
----
var server = Java.type("io.vertx.ext.shell.term.TermServer").createSSHTermServer(vertx, {
  "port" : 5000,
  "host" : "localhost"
});
...
[source,js]
----
var server = Java.type("io.vertx.ext.shell.term.TermServer").createTelnetTermServer(vertx, {
  "port" : 5000,
  "host" : "localhost"
});
server.termHandler(function (term) {
