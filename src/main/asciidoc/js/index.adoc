...
A command is created by the `link:jsdoc/command-Command.html#builder[Command.builder]` method: the command process handler is called
...
var builder = Command.builder("my-command");
...
var command = Command.builder(cli);
command.processHandler(function (process) {
