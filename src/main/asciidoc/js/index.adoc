...
A command is created by the `link:jsdoc/command_builder-CommandBuilder.html#builder[CommandBuilder.builder]` method: the command process handler is called
...
var builder = CommandBuilder.builder("my-command");
...
var command = CommandBuilder.builder(cli);
command.processHandler(function (process) {
