...
The `link:jsdoc/command_process-CommandProcess.html#interruptHandler[interruptHandler]` is called when the process is interrupted, this event is fired when the user press
...
When the size of the terminal changes the `link:jsdoc/command_process-CommandProcess.html#resizehandler[resizehandler]`
...
The `link:jsdoc/tty-Tty.html#resizehandler[resizehandler]` is useful for being noticed when the user resize the terminal
window:

[source,js]
----
term.resizehandler(function (v) {
  console.log("terminal resized : " + term.width() + " " + term.height());
});
