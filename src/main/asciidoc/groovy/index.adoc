...
A command is created by the `link:groovydoc/io/vertx/groovy/ext/shell/command/CommandBuilder.html#builder(java.lang.String)[CommandBuilder.builder]` method: the command process handler is called
...
def builder = CommandBuilder.builder("my-command")
...
def command = CommandBuilder.builder(cli)
command.processHandler({ process ->
