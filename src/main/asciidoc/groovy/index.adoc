...
A command can set a `link:groovydoc/io/vertx/groovy/ext/shell/command/CommandProcess.html#setStdin(io.vertx.ext.shell.io.Stream)[setStdin]` handler
...
When the size of the terminal changes the `link:groovydoc/io/vertx/groovy/ext/shell/command/CommandProcess.html#resizehandler(io.vertx.core.Handler)[resizehandler]`
...
The `link:groovydoc/io/vertx/groovy/ext/shell/io/Tty.html#resizehandler(io.vertx.core.Handler)[resizehandler]` is useful for being noticed when the user resize the terminal
window:

[source,groovy]
----
term.resizehandler({ v ->
  println("terminal resized : ${term.width()} ${term.height()}")
})
