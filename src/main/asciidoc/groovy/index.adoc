...
import io.vertx.groovy.ext.shell.registry.CommandResolver
...
server.commandRegistry().registerCommands(CommandResolver.baseCommands())
...
It can be achieved via the `java.util.ServiceLoader` of a `link:../../groovydoc/io/vertx/groovy/ext/shell/registry/CommandResolver.html[CommandResolver]`.
...
For shell servers, the command pack can be an argument of `link:../../groovydoc/io/vertx/groovy/ext/shell/registry/CommandRegistry.html#registerCommands(io.vertx.ext.shell.registry.CommandResolver)[registerCommands]`.