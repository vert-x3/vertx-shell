...
# create the auth config
echo user.admin=password > auth.properties
vertx run -conf '{"httpOptions":{"port":4000,"ssl":true,"keyStoreOptions":{"path":"keystore.jks","password":"secret"},"shiroAuthOptions":{"config":{"properties_path":"file:auth.properties"}}}}' maven:io.vertx:vertx-shell:3.2.0-SNAPSHOT
----
...
- `link:../dataobjects.html#SSHTermOptions#set_shiro_auth_options-instance_method[shiroAuthOptions]`: configures user authentication
...
- `link:../dataobjects.html#HttpTermOptions#set_shiro_auth_options-instance_method[shiroAuthOptions]`: configures user authentication
...
server.command_registry().register_commands(VertxShell::Command.base_commands())
...
An HTTP term can start its own HTTP server, or it can reuse an existing Vert.x Web `link:../../vertx-web/ruby/yardoc/VertxWeb/Router.html[Router]`.
...
For customization purpose these resources should be copied and customized, they are available in the Vert.x Shell
