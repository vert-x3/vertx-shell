...
or `maven:io.vertx:vertx-shell:3.1.0-SNAPSHOT`
...
A command is created by the `link:yardoc/VertxShell/CommandBuilder.html#builder-class_method[CommandBuilder.builder]` method: the command process handler is called
...
builder = VertxShell::CommandBuilder.builder("my-command")
...
command = VertxShell::CommandBuilder.builder(cli)
command.process_handler() { |process|
