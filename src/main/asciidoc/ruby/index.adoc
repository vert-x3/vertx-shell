...
A command is created by the `link:yardoc/VertxShell/Command.html#builder-class_method[Command.builder]` method: the command process handler is called
...
builder = VertxShell::Command.builder("my-command")
...
command = VertxShell::Command.builder(cli)
command.process_handler() { |process|
