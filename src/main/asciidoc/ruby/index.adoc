...
server.command_registry().register_commands(VertxShell::CommandResolver.base_commands())
...
It can be achieved via the `java.util.ServiceLoader` of a `link:../../yardoc/VertxShell/CommandResolver.html[CommandResolver]`.
...
For shell servers, the command pack can be an argument of `link:../../yardoc/VertxShell/CommandRegistry.html#register_commands-instance_method[registerCommands]`.