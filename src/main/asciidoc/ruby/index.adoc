...
The `link:yardoc/VertxShell/CommandProcess.html#interrupt_handler-instance_method[interruptHandler]` is called when the process is interrupted, this event is fired when the user press
...
When the size of the terminal changes the `link:yardoc/VertxShell/CommandProcess.html#resizehandler-instance_method[resizehandler]`
...
Vert.x Shell provides also bare terminal servers for those who need to write pure terminal applications.
...
The `link:yardoc/VertxShell/Tty.html#resizehandler-instance_method[resizehandler]` is useful for being noticed when the user resize the terminal
window:

[source,ruby]
----
term.resizehandler() { |v|
  puts "terminal resized : #{term.width()} #{term.height()}"
}
